...
namespace WebAgroCoreBCC2021.Models.Mapeamento
{
    public class InsumoAreaMap : IEntityTypeConfiguration<InsumoArea>
    {
        public void Configure(EntityTypeBuilder<InsumoArea> builder)
        {
            ...
            builder.HasOne(p => p.insumo).WithMany(p => p.areasinsumo).HasForeignKey(p => p.insumo).OnDelete(DeleteBehavior.NoAction);
